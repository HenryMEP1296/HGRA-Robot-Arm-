...
void draw() {
  background(0);
  float lJoystickY = map(controller.getSlider("leftStickVert").getValue(), -1, 1, 0, 9); // grabbing certain cont value from object and them mapping into suitable units
  float rJoystickY = map(controller.getSlider("rightStickVert").getValue(), -1, 1, 0, 9);// grabbing certain cont value from object and them mapping into suitable units
  float r2 = map(controller.getSlider("r2").getValue(), -1, 1, 0, 9);// grabbing certain cont value from object and them mapping into suitable units
  ...
  String totalVal = str(int(lJoystickY)) + str(int(rJoystickY)) + str(int(r2)) + str(int(l1)) + str(int(r1)) + str(int(circle)) + str(int(square)); // toalt string

  println(totalVal); // test print
  myPort.write(totalVal); //writing r2 to the serial port
}
